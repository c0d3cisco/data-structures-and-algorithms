...
const updateAnimal = (arr, callback) => {
  const newArr = [];
  arr.forEach(arrItem => newArr.push(callback(arrItem)));
  return newArr;
};
...
const alphabetizeBetter = (arr) => {
  const newArr = arr.sort((a, b) => {
    a = a.toLowerCase();
    b = b.toLowerCase();
    return a > b ? 1 : (a < b ? -1 : 0); // checks if a is greater than b - if true then 1; if false compare if a is less than b - if true then -1; if false then 0.
  });
  return newArr;
};
...
const sortPeople = (arr) => {
  return arr.sort((a, b) => {
    a = a.lastName;
    b = b.lastName;
    return a > b ? 1 : (a < b ? -1 : 0); //  why doesnt this work? return arr.sort( (a,b) => {return a.lastName-b.lastName; });
  });
};
...
const sortSchedule = (arr) => {
  return arr.sort((a, b) => {
    ...
    const aTime = a.end - a.start;
    const bTime = b.end - b.start;
    const aDay = (a.dayOfWeek === 'Monday') ? 1 : ((a.dayOfWeek === 'Tuesday') ? 2 : ((a.dayOfWeek === 'Wednesday') ? 3 : ((a.dayOfWeek === 'Friday') ? 4 : 0)));
    const bDay = (b.dayOfWeek === 'Monday') ? 1 : ((b.dayOfWeek === 'Tuesday') ? 2 : ((b.dayOfWeek === 'Wednesday') ? 3 : ((b.dayOfWeek === 'Friday') ? 4 : 0)));
    if (aDay > bDay) {
      return 1;
    } else if (aDay < bDay) {
      return -1;
