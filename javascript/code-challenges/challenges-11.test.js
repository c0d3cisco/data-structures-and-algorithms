...
function transformToLis(obj){
  let arr = [];
  Object.entries(obj).forEach(item => arr.push(`<li>${item[0]}: ${item[1]}</li>`));//`<li>${item[0]}: ${item[1]}</li>`);
  return arr;
}
...
const count = (target, input) => input.flat().reduce((acc, curr) => curr === target ? acc + 1 : acc, 0);
...
const totalSum = (input) => input.flat().reduce((acc, curr) => acc + curr, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  const transArr = [];
  input.forEach(row => {
    const iterateArr = row.filter(num => (typeof num === typeof 0) && (num % 5 === 0));
    transArr.push(iterateArr.map(num => (typeof num === typeof 0) ? Math.pow(2, num) : []));
  });
  return transArr;
};
...
let findMaleAndFemale = (data) => {
  const arr1 = data.filter(obj => obj.gender === 'male' || obj.gender === 'female');
  const arr2 = arr1.map(obj => obj.name).join(' and ');
  console.log(arr2);
  return arr2;
};
