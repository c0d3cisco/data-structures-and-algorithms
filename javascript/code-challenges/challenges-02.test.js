...
const raisedToTheThird = (arr) => {
  arr.forEach(arrItem => {
    const numHold = Math.pow(arrItem, 3);
    newArr.push(numHold);
  });
  return newArr;
};
...
const extractAbilities = (arr) => {
  let listAbilitiesNames = arr.map(arrItem => arrItem.ability.name);
  return listAbilitiesNames;
};
...
const extractStats = (arr) => {
  let statsInfo = arr.map(statInfo => {
    return ({name: statInfo.stat.name, total: statInfo.effort + statInfo.baseStat});
  });
  return statsInfo;
};
