...
const screenForNames = (arr) => {
  ...
  const newArr = arr.filter(name => regex.test(name));
  return newArr;
};
...
const toTitleCase = (arr) => {
  const arrNew = arr.map(string => {
    let stringArr = string.split('');
    stringArr[0] = stringArr[0].toUpperCase();
    return string = stringArr.join('');
  });
  return arrNew;
};
...
let biggerThanLuke = (arr) => {
  let arrNew = arr.filter(character => parseInt(arr[0].mass) < parseInt(character.mass));
  return arrNew = arrNew.map(character => character.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  const checkThree = (x,y,z) => {
    return (x === y && y === z && x !== '') ? true : false;
  };
  const fb = board.flat();
  const op1 = checkThree(fb[0], fb[1], fb[2]);
  const op2 = checkThree(fb[3], fb[4], fb[5]);
  const op3 = checkThree(fb[6], fb[7], fb[8]);
  const op4 = checkThree(fb[0], fb[3], fb[6]);
  const op5 = checkThree(fb[1], fb[4], fb[7]);
  const op6 = checkThree(fb[2], fb[5], fb[8]);
  const op7 = checkThree(fb[0], fb[4], fb[8]);
  const op8 = checkThree(fb[2], fb[4], fb[6]);

  return (op1 || op2 || op3 || op4 || op5 || op6 || op7 || op8) ? true : false;
};
