...
const longestString = (arr) => {
  let maxIndex = arr.map((arrItem, idx) => [arrItem.length, idx]).sort((a, b) => b[0] - a[0])[0];
  return maxIndex ? maxIndex[1] : -1;
};
...
const standardizePhoneNumbers = (arr) => arr.map(num => num.replace(/\W/g,''));
...
const onlyOddChars = (str) => str.split('').filter((item, idx) => idx%2 ? item : false).join('');
