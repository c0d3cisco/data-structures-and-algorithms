...
const toLastNames = people => {
  const newPeople = people.map((people => people.firstName + ' ' + people.lastName));
  return newPeople;
};
...
const addValues = (arr) => {
  const arrAdd = arr.reduce((acc, currentV) => acc + currentV, 0);
  return arrAdd;
};
...
const addPurchases = (arr) => {
  const purchaseTotal = arr.reduce((accumulator, iterativeValue) => accumulator + iterativeValue.purchasePrice, 0);
...
const countNumberOfElements = (arr) => {
  const totalElements = arr.reduce((accumulator) => accumulator + 1, 0);
...
const returnNames = (arr) => {
  const arrNames = arr.reduce((accumulator, iterativeValue) => {
    accumulator.push(iterativeValue.name);
    return accumulator;
  }, []);
  return arrNames;
};
...
const reversedString = (str) => {
  const strArr = [str];
  const reverseString = strArr.reduce((accumulator, iterativeValue) => {
    for (let i = 0; i < str.length; i++) {
      accumulator = accumulator + iterativeValue[str.length - 1 - i];
...
const countNumberOfChildren = (arr) => {
  const totalChildren = arr.reduce((acc,inValue) => {
    if(inValue.children){
      acc = acc + inValue.children.length;
    }
    return acc;
  },0);
...
const calculateAverage = (arr) => {
  const arrAdd = arr.reduce((acc, currentV) => acc + currentV, 0);
  const arrAvg = arrAdd/arr.length;
  return arrAvg;
};
...
const countPrimeNumbers = (arr) => {
  const primeArray = arr.reduce((acc, curr) => isPrime(curr) ? (acc + 1) : acc, 0);
  return primeArray;
};
...
const extractStats = (snorlaxData) => {
  const extArr = snorlaxData["stats"].reduce((acc, curr) => {
...
const extractChildren = (arr) => {
  const newArr = arr.filter(arrItem => {if(arrItem.children) return arrItem.name.includes('a');});
  console.log(newArr);
  const childrenArr = newArr.reduce((acc, curr) => [...acc, ...curr.children],[]);
