...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((acc,currentV) => acc > currentV ? acc : currentV, 0);
...
const checkValues = (obj, value) => Object.values(obj).find(item => item === value) ? true : false;
...
const updateNumbers = (obj) => Object.keys(obj).map((employee, idx) => employee + ': ' + Object.values(obj)[idx]);
...
const getHouses = (arr) => arr.reduce((acc, current) => {
  acc.push(current.house);
  return acc;
}, []);
...
const hasChildrenValues = (arr, character) => arr.find(char => Object.values(char)[0] === character).children ? true : false;
